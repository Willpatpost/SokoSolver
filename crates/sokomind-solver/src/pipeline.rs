...
use crate::search::beam::{
    beam_search, goal_packing_count, sum_min_distances, BeamConfig, BeamResult,
};
...
impl<'a> EndgameContext<'a> {
    ...
    fn run_phase_a(&mut self) -> Option<SearchOutcome> {
        ...
        let astar_candidates: Vec<usize> = self
            .checkpoints
            .iter()
            .enumerate()
        ...
        self.logger.log(
            LogLevel::Debug,
            &format!(
                "  endgame quick A*: {} checkpoints (pack >= {}, {}ms each)",
                astar_candidates.len(),
                astar_threshold,
                astar_per_ms
            ),
        );
            ...
            match astar_result {
                AStarResult::Solved { pushes: suffix, .. } => {
                    self.logger.log(
                        LogLevel::Info,
                        &format!(
                            "    A* solved! suffix={} pushes (expanded={})",
                            suffix.len(),
                            astar_counters.expanded
                        ),
                    );
                ...
                _ => {
                    self.logger.log(
                        LogLevel::Info,
                        &format!(
                            "    A* not solved (expanded={}, generated={}, elapsed={:.0}ms)",
                            astar_counters.expanded,
                            astar_counters.generated,
                            astar_budget.elapsed_ms()
                        ),
                    );
                }
            }
    ...
    fn run_phase_a2(&mut self) -> Option<SearchOutcome> {
        ...
        let focus_candidates: Vec<usize> = self
            .checkpoints
            .iter()
            .enumerate()
        ...
        self.logger.log(
            LogLevel::Debug,
            &format!(
                "  Phase A2: focused A* ({} cp x {} cfg, {}ms each, cap={:.0}ms)",
        ...
        for &ci in &focus_candidates {
            if self.budget.exhausted() || self.budget.elapsed_ms() >= a2_deadline {
                break;
            }
            let checkpoint = self.checkpoints[ci].0.clone();
            let prefix_pushes = self.checkpoints[ci].1.clone();
            ...
            for fc in focus_configs {
                ...
                self.logger.log(
                    LogLevel::Debug,
                    &format!(
                        "    focused A* cp={} pack={} h={} radius={} w={}",
                        ci, pack, h, fc.radius, fc.weight
                    ),
                );
                ...
                match endgame_focused_search(
                    ...
                    EndgameResult::Solved { pushes: suffix } => {
                        self.logger.log(
                            LogLevel::Info,
                            &format!(
                                "    SOLVED! suffix={} pushes (expanded={}, generated={})",
                                suffix.len(),
                                focus_counters.expanded,
                                focus_counters.generated
                            ),
                        );
    ...
    fn run_phase_a3(&mut self) -> Option<SearchOutcome> {
        ...
        let dfs_candidates: Vec<usize> = self
            .checkpoints
            .iter()
            .enumerate()
        ...
        self.logger.log(
            LogLevel::Debug,
            &format!(
                "  Phase A3: greedy DFS ({} cp, {} base cfg x {} seeds, {}ms each, cap={:.0}ms)",
                dfs_candidates.len(),
                base_configs.len(),
                noise_seeds.len(),
                dfs_per_ms,
    ...
    fn run_phase_b1(&mut self) -> Option<SearchOutcome> {
        ...
        self.logger.log(
            LogLevel::Debug,
            &format!(
                "  Phase B1: {} wide rescue beams ({:.0}ms each, {:.0}ms budget)",
                wide_total, wide_per_ms, wide_budget_ms
            ),
        );
    ...
    fn run_phase_b2(&mut self) -> Option<SearchOutcome> {
        ...
        let narrow_seeds: &[u64] = &[
            0, 7919, 31337, 104729, 65537, 999983, 314159, 271828, 1000003, 2000003, 3000017,
            4000037, 5000003, 6000011, 7000003, 8000009, 9000011, 1234567, 7654321, 2718281,
            3141592, 1618033, 4669201, 6931471, 1414213, 1732050, 2236067, 2645751, 3162277,
            3316624, 3605551, 3872983,
        ];
        ...
        self.logger.log(
            LogLevel::Debug,
            &format!(
                "  Phase B2: {} narrow restarts from best checkpoint (pack={}, h={}, {:.0}ms each)",
                max_narrow, best_pack, best_h, narrow_per_ms
            ),
        );
        ...
        for (ni, &seed) in narrow_seeds.iter().take(max_narrow).enumerate() {
            ...
            self.logger.log(
                LogLevel::Debug,
                &format!(
                    "    narrow {} w={} hw={:.1} seed={} deadline={:.0}ms",
                    ni, w, hw, seed, attempt_deadline
                ),
            );
    ...
    fn run_phase_c(&mut self) -> Option<SearchOutcome> {
        ...
        self.high_pack_checkpoints
            .dedup_by(|(a, _), (b, _)| a.box_cells == b.box_cells);
        ...
        let best_focus_pack = self
            .high_pack_checkpoints
            .first()
        ...
        for attempt in 0..max_attempts {
            ...
            if attempt < 10 || attempt % 20 == 0 {
                ...
                self.logger.log(
                    LogLevel::Debug,
                    &format!(
                        "    focus #{} cp={} w={} div={:.1} seed={} pack={} h={} deadline={:.0}ms",
                        attempt, ci, w, div, seed, pack, h, attempt_deadline
                    ),
                );
            }
...
fn try_beam_large(
        ...
        for (ci, config) in configs.iter().enumerate() {
            ...
            ctx.logger.log(
                ...
                &format!(
                    ...
                    cfg.beam_width,
                    cfg.seed,
                    cfg.goal_packing_weight,
                    cfg.typed_packing_weight,
            ...
            match result {
                ...
                BeamResult::Checkpoints { states } => {
                    ctx.logger.log(
                        LogLevel::Debug,
                        &format!("    config {} produced {} checkpoints", ci, states.len()),
                    );
    ...
    // Extra beam restarts with varied scoring profiles.
    {
        ...
        ctx.logger.log(
            LogLevel::Debug,
            &format!(
                "  extra beams from initial: {} configs, {:.0}ms each, {:.0}ms budget",
                extra_configs.len(),
                extra_per_ms,
                extra_phase_budget
            ),
        ...
        for ec in extra_configs {
            if ctx.budget.exhausted() || ctx.budget.elapsed_ms() > extra_deadline {
                break;
            }
            ...
            ctx.logger.log(
                LogLevel::Debug,
                &format!(
                    "    extra w={} s={} hw={:.1} wp={:.1} ga={:.1} div={:.1} deadline={:.0}ms",
                    ec.width, ec.seed, ec.hw, ec.wp, ec.ga, ec.div, attempt_deadline
                ),
            );
    ...
    ctx.logger.log(
        LogLevel::Debug,
        &format!(
            "  endgame: {} unique checkpoints, best_pack={}",
            ctx.checkpoints.len(),
            best_pack
        ),
    );
    ...
    if let Some(outcome) = ctx.run_phase_a() {
        return Some(outcome);
    }
    if let Some(outcome) = ctx.run_phase_a2() {
        return Some(outcome);
    }
    if let Some(outcome) = ctx.run_phase_a3() {
        return Some(outcome);
    }
    if let Some(outcome) = ctx.run_phase_b1() {
        return Some(outcome);
    }
    if let Some(outcome) = ctx.run_phase_b2() {
        return Some(outcome);
    }
    if let Some(outcome) = ctx.run_phase_c() {
        return Some(outcome);
    }
...
fn try_beam(
    ...
    if box_count >= 12 {
        return try_beam_large(
            cb, initial, zk, heuristic, deadlocks, macros, plan_ref, budget, counters, logger,
        );
    }
    ...
    let profiles: &[(usize, u64, u32)] = if box_count >= 8 {
        ...
        &[(128, 0, 500), (256, 7919, 500), (512, 31337, 500)]
    };
        ...
        match beam_search(
            ...
            BeamResult::Checkpoints { states } => {
                ...
                logger.log(
                    LogLevel::Debug,
                    &format!(
                        "  launching endgame from {} checkpoints (of {})",
                        n_checkpoints,
                        states.len()
                    ),
                );
                ...
                for (ci, (checkpoint, prefix_pushes)) in
                    states.iter().take(n_checkpoints.min(4)).enumerate()
                {
                    if budget.exhausted() {
                        break;
                    }
                    ...
                    logger.log(
                        LogLevel::Debug,
                        &format!(
                            "    A* from checkpoint {} (prefix={})",
                            ci,
                            prefix_pushes.len()
                        ),
                    );
                    let astar_result =
                        astar_search(cb, checkpoint, zk, heuristic, deadlocks, budget, counters);
                    match astar_result {
                        AStarResult::Solved { pushes: suffix, .. } => {
                            logger.log(
                                LogLevel::Info,
                                &format!("    A* solved! suffix={} pushes", suffix.len()),
                            );
                ...
                for (ci, (checkpoint, prefix_pushes)) in
                    states.iter().take(n_checkpoints).enumerate()
                {
